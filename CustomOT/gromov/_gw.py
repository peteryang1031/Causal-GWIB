...
def fused_gromov_wasserstein_our(M, C_1, C_2, C_Phi_1, C_Phi_2, p=None, q=None, loss_fun='square_loss', symmetric=None, beta=0.5,
    ...
    **kwargs : dict
    ...
    nx = get_backend(*arr)
    p0, q0, C10, C20, CP10, CP20, M0, beta0 = p, q, C_1, C_2, C_Phi_1, C_Phi_2, M, beta
    ...
    def f(G):
        return beta * gwloss(constC_1, hC1_1, hC2_1, G, np_) + gwloss(constC_2, hC1_2, hC2_2, G, np_) \
            + gwloss(constC_3, hC1_3, hC2_3, G, np_)

    # if symmetric:
    def df(G):
        return beta * gwggrad(constC_1, hC1_1, hC2_1, G, np_) + gwggrad(constC_2, hC1_2, hC2_2, G, np_) \
                + gwggrad(constC_3, hC1_3, hC2_3, G, np_)
    ...
    if armijo:
        ...
        def line_search(cost, G, deltaG, Mi, cost_G, **kwargs):
            return solve_gromov_linesearch_our(G, deltaG, cost_G, C_1, C_2, C_Phi_1, C_Phi_2, M= (1-beta) * M, reg=beta, nx=np_, **kwargs)
    
    if log:
        res, log = cg_our(p, q, (1-beta) * M, beta, f, df, G0, line_search, log=True, numItermax=max_iter, stopThr=tol_rel, stopThr2=tol_abs, **kwargs)
        log['fgw_dist'] = nx.from_numpy(log['loss'][-1], type_as=C10)
        log['u'] = nx.from_numpy(log['u'], type_as=C10)
        log['v'] = nx.from_numpy(log['v'], type_as=C10)
        return nx.from_numpy(res, type_as=C10), log
    else:
        return nx.from_numpy(cg_our(p, q, (1-beta) * M, beta, f, df, G0, line_search, log=False, numItermax=max_iter, stopThr=tol_rel, stopThr2=tol_abs, **kwargs), type_as=C10)


def fused_gromov_wasserstein2_our(M, C_1, C_2, C_Phi_1, C_Phi_2, p=None, q=None, loss_fun='square_loss', symmetric=None, beta=0.5,
                              armijo=False, G0=None, log=False, max_iter=1e4, tol_rel=1e-9, tol_abs=1e-9, **kwargs):
    r"""
    ...
    **kwargs : dict
    ...
    T, log_fgw = fused_gromov_wasserstein_our(
        M, C_1, C_2, C_Phi_1, C_Phi_2, p, q, loss_fun, symmetric, beta, armijo, G0, log=True,
        max_iter=max_iter, tol_rel=tol_rel, tol_abs=tol_abs, **kwargs)
    ...
    if loss_fun == 'square_loss':
        ...
        gC_1 += 2 * C_1 * nx.outer(p, p) - 2 * nx.dot(T, nx.dot(C_Phi_2, T.T))
        gC_Phi_2 += 2 * C_Phi_2 * nx.outer(q, q) - 2 * nx.dot(T.T, nx.dot(C_1, T))
        
        gC_Phi_1 += 2 * C_Phi_1 * nx.outer(p, p) - 2 * nx.dot(T, nx.dot(C_2, T.T))
        gC_2 += 2 * C_2 * nx.outer(q, q) - 2 * nx.dot(T.T, nx.dot(C_Phi_1, T))
        
        lin_term = nx.sum(T * M)
        gw_term = (fgw_dist - (1 - beta) * lin_term) / beta
        fgw_dist = nx.set_gradients(fgw_dist, (p, q, C_1, C_2, C_Phi_1, C_Phi_2, M, beta),
...
def solve_gromov_linesearch_our(G, deltaG, cost_G, C_1, C_2, C_Phi_1, C_Phi_2, M,reg,
    ...
    a_1 = reg * -2 * nx.sum(dot_1 * deltaG)
    b_1 = reg * -2 * (nx.sum(dot_1 * G) + nx.sum(nx.dot(nx.dot(C_Phi_1, G), C_Phi_2.T) * deltaG))
    
    dot_2 = nx.dot(nx.dot(C_1, deltaG), C_Phi_2.T)
    a_2 = -2 * nx.sum(dot_2 * deltaG)
    b_2 = -2 * (nx.sum(dot_2 * G) + nx.sum(nx.dot(nx.dot(C_1, G), C_Phi_2.T) * deltaG))
    
    dot_3 = nx.dot(nx.dot(C_Phi_1, deltaG), C_2.T)
    a_3 = -2 * nx.sum(dot_3 * deltaG)
    b_3 = -2 * (nx.sum(dot_3 * G) + nx.sum(nx.dot(nx.dot(C_Phi_1, G), C_2.T) * deltaG))
